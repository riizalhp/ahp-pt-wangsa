...
use App\Services\Migration\MigrationMapper;
...
return new class extends Migration
{
     ...
     *
     * The per-row field mapping lives in the pure
     * App\Services\Migration\MigrationMapper helper, keeping this migration
     * thin and the mapping logic independently testable (no DB, no Eloquent).
     *
     ...
     * `data_pengadaan` table is untouched.
     */
    public function down(): void
    {
        ...
        DB::transaction(function () use ($migratedHeaderIds) {
            // Remove the detail rows explicitly first, then their headers, so
            // the reversal is safe regardless of whether the FK cascade fires
            // (e.g. on database engines where it is not enforced).
