<x-layouts.app title="Ubah Data Kriteria">
    <div class="mb-6">
    ...
    <div class="max-w-2xl bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <form action="{{ route('supervisor.kriteria.update', $kriterium->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="kode" id="kode" required value="{{ old('kode', $kriterium->kode) }}"
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" required value="{{ old('nama', $kriterium->nama) }}"
                           class="block w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-teal focus:ring-4 focus:ring-teal/15 transition-all outline-none text-sm text-slate-800 font-medium"
                           placeholder="Contoh: Cost">
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3"
                          class="block w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-teal focus:ring-4 focus:ring-teal/15 transition-all outline-none text-sm text-slate-800 font-medium"
                          placeholder="Masukkan rincian penjelasan kriteria">{{ old('deskripsi', $kriterium->deskripsi) }}</textarea>
