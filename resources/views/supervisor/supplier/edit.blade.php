<x-layouts.app title="Ubah Data Supplier">
    <div class="mb-6">
    ...
    <div class="max-w-2xl bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <form action="{{ route('supervisor.supplier.update', $supplier->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="kode" id="kode" required value="{{ old('kode', $supplier->kode) }}"
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" required value="{{ old('nama', $supplier->nama) }}"
                           class="block w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-teal focus:ring-4 focus:ring-teal/15 transition-all outline-none text-sm text-slate-800 font-medium"
                ...
                <div>
                    ...
                    <input type="text" name="jenis_barang" id="jenis_barang" required value="{{ old('jenis_barang', $supplier->jenis_barang) }}"
                           class="block w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-teal focus:ring-4 focus:ring-teal/15 transition-all outline-none text-sm text-slate-800 font-medium"
                ...
                <div>
                    ...
                    <input type="number" name="lama_kerja_sama" id="lama_kerja_sama" min="0" value="{{ old('lama_kerja_sama', $supplier->lama_kerja_sama) }}"
                           class="block w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-teal focus:ring-4 focus:ring-teal/15 transition-all outline-none text-sm text-slate-800 font-medium"
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="3"
                          class="block w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-teal focus:ring-4 focus:ring-teal/15 transition-all outline-none text-sm text-slate-800 font-medium"
                          placeholder="Masukkan alamat fisik supplier">{{ old('alamat', $supplier->alamat) }}</textarea>
